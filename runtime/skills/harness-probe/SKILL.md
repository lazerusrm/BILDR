---
name: harness-probe
description: Use BILDR's bounded local repository probe to batch search, read-many, workspace mapping, test selection, and log summarization without flooding model context.
---

# Repository probe

Use `harness-probe` before making many repetitive `rg`, `cat`, or metadata calls.
It is read-only and stores full outputs as BILDR artifacts while returning a
...
harness-probe cargo-map --affected crates/service/src/api.rs --format json

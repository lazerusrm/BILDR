...
impl Orchestrator {
    pub async fn new(
        ...
        // Native subagent activity is durable source evidence. Never
        // reconstruct or rewrite child sessions from it before restart
        // reconciliation has recorded its immutable inventory and receipts.
        // Persisted child sessions are reconciled below; unbound activity
        // remains visible for an explicit future reconciliation action.
        orchestrator
            .projection
...
mod tests {
    ...
    async fn terminal_investigation_replay_repairs_a_prior_deadline_failure() {
        ...
        orchestrator
            ...
            .await
            .expect(
                "restart reconciliation repairs the artifact-backed failure without a callback",
            );
        assert_eq!(
            orchestrator
                .store()
        ...
        orchestrator
            ...
            .expect(
                "terminal lifecycle can be split after the immutable artifact remains committed",
            );

        orchestrator
            .accept_investigation_response(&investigator.id, &response_json)
            .await
            .expect("exact terminal replay restores artifact-backed lifecycle");
        assert_eq!(
            orchestrator
                .store()
                .task(&task_id)
    ...
    async fn investigation_response_after_deadline_is_rejected_before_maintenance_runs() {
        ...
        let response = json!({
            "schema": "harness.investigation-response.v1",
            "methods": ["late response"],
            "findings": [{
                "finding_id": "finding-1",
                "classification": "supported",
                "summary": "A valid response must still lose to the durable deadline.",
                "confidence_milli": 900,
                "evidence_refs": [investigation_context_evidence_ref(&context_digest)],
                "affected_refs": [],
                "risk": "normal",
        ...
        orchestrator
            ...
            .put_runtime_metadata(
                ...
                &json!({
                    ...
                    "attempt_id": "forged-attempt-binding"
                }),
            )
            .expect("malformed deadline binding persists");
        let response = json!({
            "schema": "harness.investigation-response.v1",
            "methods": ["attempt malformed deadline acceptance"],
            "findings": [{
                "finding_id": "finding-1",
                "classification": "supported",
                "summary": "The response must not outlive a malformed deadline binding.",
                "confidence_milli": 900,
                "evidence_refs": [investigation_context_evidence_ref(&context_digest)],
                "affected_refs": [],
                "risk": "normal",
    ...
    async fn investigation_write_or_candidate_is_rejected_before_artifact_admission() {
        ...
        .expect("fixture creates an untracked candidate request");
        let context_digest = orchestrator
            .store()
            .context_packet_digest_for_attempt(&attempt_id, "investigator")
            .expect("context reads")
        ...
        let response = json!({
            "schema": "harness.investigation-response.v1",
            "methods": ["attempt mutable candidate admission"],
            "findings": [{
                "finding_id": "finding-1",
                "classification": "supported",
                "summary": "A changed investigation worktree must never become an immutable artifact.",
                "confidence_milli": 900,
                "evidence_refs": [investigation_context_evidence_ref(&context_digest)],
                "affected_refs": [],
                "risk": "normal",
